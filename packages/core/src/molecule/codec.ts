...
import { Num, numBeFromBytes, numBeToBytes, numFromBytes, numToBytes } from "../num/index.js";
 ...
 * @param codec
 */
export function bytes<Encodable>(
    ...
    return {
        ...
        encode: (userDefined) => {
            ...
            if (encoded.byteLength !== byteLength) {
                throw new Error(`fixedBytes: invalid encode byte length of ${encoded.byteLength}`);
            }
            return encoded;
        },
        decode: (buffer) => {
            ...
            if (value.byteLength !== byteLength) {
                throw new Error(`fixedBytes: invalid decode byte length of ${value.byteLength}`);
            }
 ...
 * @param littleEndian
 */
export const uint = (byteLength: number, littleEndian = false) => {
