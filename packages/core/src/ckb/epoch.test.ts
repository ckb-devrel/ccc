...
describe("Epoch", () => {
  it("constructs from tuple and object via from()", () => {
    ...
    const c = Epoch.from([7n, 8n, 9n]);
    expect(Epoch.from(c)).toBe(c);
  });

  it("packs and unpacks numeric layout (toNum/fromNum) and hex conversion", () => {
    const e = Epoch.from([0x010203n, 0x0405n, 0x0607n]); // use values within bit widths
  ...
  it("throws when packing negative components with toNum", () => {
    const e = Epoch.from([-1n, 0n, 1n]);
    expect(() => e.toNum()).toThrow();

    const e2 = Epoch.from([0n, -1n, 1n]);
    expect(() => e2.toNum()).toThrow();

    const e3 = Epoch.from([0n, 0n, -1n]);
    expect(() => e3.toNum()).toThrow();
  });

  it("throws when packing components too big with toNum", () => {
    const e = Epoch.from([1n << 24n, 1n, 1n]); // integer = 16777215 (24-bit limit + 1)
    expect(() => e.toNum()).toThrow();

    const e2 = Epoch.from([1n, 1n << 16n, 1n]); // numerator = 65536 (16-bit limit + 1)
    expect(() => e2.toNum()).toThrow();

    const e3 = Epoch.from([1n, 1n, 1n << 16n]); // denominator = 65536 (16-bit limit + 1)
    expect(() => e3.toNum()).toThrow();
  });

  it("normalizeBase fixes zero or negative denominators", () => {
    const a = Epoch.from([1n, 2n, 0n]).normalizeBase();
    expect(a.denominator).toBe(1n);
    expect(a.numerator).toBe(0n);

    const b = Epoch.from([1n, 2n, -3n]).normalizeBase();
  ...
  it("comparison operations and compare()", () => {
    const a = Epoch.from([1n, 0n, 1n]);
    const b = Epoch.from([1n, 1n, 2n]);
    const c = Epoch.from([2n, 0n, 1n]);
    ...
    expect(b.eq(Epoch.from([1n, 2n, 4n]))).toBe(true); // 1 + 1/2 == 1 + 2/4
    expect(c.gt(b)).toBe(true);
    expect(c.ge(b)).toBe(true);
  });

  it("add and sub arithmetic with differing denominators", () => {
    const a = Epoch.from([1n, 1n, 2n]); // 1.5
    const b = Epoch.from([2n, 1n, 3n]); // 2 + 1/3
    const s = a.add(b);
    ...
    const sub = s.sub(Epoch.from([1n, 5n, 6n]));
  ...
  it("toUnix estimates timestamp using a reference header", () => {
    const refEpoch = Epoch.from([1n, 0n, 1n]);
    ...
    const target = Epoch.from([2n, 1n, 2n]);
