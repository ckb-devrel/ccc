...
describe("Transaction", () => {
  ...
  describe("completeInputsByUdt", () => {
    ...
    it("should not collect a prefix-matching UDT with different args", async () => {
      ...
      let receivedSearchKey: Parameters<typeof client.findCells>[0] | undefined;
      vi.spyOn(client, "findCells").mockImplementation(
        async function* (searchKey) {
          if (!searchKey.filter?.script) {
            return;
          }
          receivedSearchKey = searchKey;
          yield mockUdtCells[0];
          const [lower, upper] = searchKey.filter.scriptLenRange!;
      ...
      expect(receivedSearchKey?.filter?.scriptLenRange).toEqual([
        type.occupiedSize,
        type.occupiedSize + 1,
      ]);
      expect(addedCount).toBe(1);
