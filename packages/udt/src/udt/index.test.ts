...
describe("Udt", () => {
  ...
  describe("complete method with capacity handling", () => {
    ...
    it("should handle capacity tweak parameter in completeInputs", async () => {
      const tx = ccc.Transaction.from({
        outputs: [{ lock, type }],
      ...
      const addedCount = await udt.completeInputs(
        tx,
        signer,
    ...
    it("should handle the two-phase capacity completion in complete method", async () => {
      ...
      // Track the calls to completeInputs to verify two-phase completion
      const completeInputsSpy = vi.spyOn(udt, "completeInputs");
      ...
      // Should have called completeInputs twice:
