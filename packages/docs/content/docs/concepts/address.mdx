...
`Address.fromString` parses an encoded CKB address into a structured object. If
the string is not an encoded address, the client's optional `addressResolver`
can resolve another representation, such as a name:
...
For encoded addresses, it validates that the prefix matches the client's
network and throws on a mismatch (e.g. passing a `ckt` address to a mainnet
client).

Passing a prefix-keyed record of clients is deprecated. When you need to handle
both networks in the same code path, try the clients explicitly and handle the
failed attempt at the call site:
...
try {
  try {
    address = await ccc.Address.fromString(someAddress, mainnetClient);
  } catch {
    address = await ccc.Address.fromString(someAddress, testnetClient);
  }
} finally {
...
### Resolve names and other representations

Configure an `AddressResolver` when an application accepts representations
other than encoded CKB addresses:

```ts
const names: Record<string, ccc.ScriptLike> = {
  "alice.example": aliceLock,
};

const addressResolver: ccc.AddressResolver = {
  shouldResolve: (value) => value.endsWith(".example"),
  resolve: async (value) => names[value],
};

const clientOwner = ccc.ClientPublicMainnet.open({ addressResolver });
const address = await ccc.Address.fromString(
  "alice.example",
  clientOwner.value,
);
...
The resolver is consulted only after normal address parsing fails.
`shouldResolve` selects the representations it owns. If it returns `false`,
`Address.fromString` preserves the original parsing error; if `resolve` returns
`undefined`, it throws `Address ... not found`. A resolved script is paired with
the supplied client's network prefix.
...
For an encoded address, `Address.fromString` combines
`addressPayloadFromString` + prefix validation + `addressFromPayload`. If the
first step fails, it may instead delegate to the client's `addressResolver`.
