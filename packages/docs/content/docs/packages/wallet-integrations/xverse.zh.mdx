...
`getXverseSigners(client)` 从 `window.btc_providers` 读取数据。Xverse 及支持 WBIP 网络方法的 Provider 会分别获得 Mainnet、Testnet4、Signet 和 Regtest 条目；旧 Provider 保留单个 `BTC` 条目。
...
| 方法 | 说明 |
| ------------------------- | ----------- |
| `connect()` | 请求授权，并切换至所选网络 |
| `disconnect()` | 清除缓存的地址 |
| `isConnected()` | 检查选定网络并静默读取支付地址，不触发授权弹窗 |
