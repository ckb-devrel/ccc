...
UniSat exposes `BTC`, `BTC Testnet`, `BTC Testnet4`, `BTC Signet`, `Fractal Bitcoin`, and `Fractal Bitcoin Testnet` signers. On `connect()`, the selected signer switches the wallet to its fixed network.
...
sequenceDiagram
    ...
    Signer->>Signer: Compare with selected network
    alt Network mismatch
        Signer->>Wallet: switchNetwork("livenet" | "testnet") or switchChain(chain)
    end
```

Testnet4, Signet, and Fractal Bitcoin require the wallet's `switchChain` API.
