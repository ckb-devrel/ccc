...
description: 将 UniSat Wallet 接入 CKB，并通过统一的 Signer 接口实现 Bitcoin 签名。
---

import { PackageBadges } from '@/components/package-badges';

`@ckb-ccc/uni-sat` 将 [UniSat Wallet](https://unisat.io/) 集成至 CCC，提供用于 Bitcoin 签名的 `SignerBtc` 实现。它通过浏览器注入的 `window.unisat` 对象与钱包通信，并支持在 Bitcoin 主网和测试网之间自动切换网络。
...
`getUniSatSigners(client)` 检查 `window.unisat` 是否存在，并为每个支持的网络返回一个 `SignerInfo`——钱包不可用时返回空数组。
...
UniSat 分别提供 `BTC`、`BTC Testnet`、`BTC Testnet4`、`BTC Signet`、`Fractal Bitcoin` 和 `Fractal Bitcoin Testnet` Signer。调用 `connect()` 时，所选 Signer 会将钱包切换到其固定网络。
...
Testnet4、Signet 和 Fractal Bitcoin 需要钱包提供 `switchChain` API。
