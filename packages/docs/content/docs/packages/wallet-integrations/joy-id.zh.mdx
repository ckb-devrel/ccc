...
new BitcoinSigner(
  ...
  network?: "btc" | "btcTestnet"                 // 默认为 "btc"
)
```
...
Factory 会为每种 BTC 地址模式分别返回 Mainnet 和 Testnet 版本。
...
|------|------------|
| `"CKB"` | `CkbSigner` |
| `"BTC"` / `"BTC Testnet"` | `BitcoinSigner`（auto） |
| `"Nostr"` | `NostrSigner` |
| `"EVM"` | `EvmSigner` |
| `"BTC (P2WPKH)"` / `"BTC Testnet (P2WPKH)"` | `BitcoinSigner`（p2wpkh） |
| `"BTC (P2TR)"` / `"BTC Testnet (P2TR)"` | `BitcoinSigner`（p2tr） |
