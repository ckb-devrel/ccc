...
`getUtxoGlobalSigners(client)` checks for `window.utxoGlobal` and returns CKB, four fixed-network BTC signers, and two fixed-network Dogecoin signers — or an empty array if the wallet isn't available.

```mermaid
graph TD
    Start["getUtxoGlobalSigners(client)"] --> Check["window.utxoGlobal exists?"]
    Check -->|No| Empty["return []"]
    Check -->|Yes| Create["Create fixed-network SignerInfo entries"]

    Create --> CKB["SignerCkb (name: 'CKB')"]
    Create --> BTC["BTC / BTC Testnet / Testnet4 / Signet"]
    Create --> DOGE["DOGE / DOGE Testnet"]
