...
| `locale`            | `ccc.ConnectorLocale`                                                  | 界面语言（BCP 47 标签，如 `"zh-CN"`），默认 `"en"` |
...
| `locale`     | `ccc.ConnectorLocale`          | 连接器当前使用的界面语言                                |
...
内置语言：`en`、`zh-CN`。未知标签会回退到同语言中最接近的已注册语言（`zh-HK` → `zh-CN`），再回退到英文，因此直接传 `navigator.language` 也是安全的。钱包与链名（`MetaMask`、`JoyID`、`BTC` 等）是品牌名，不做翻译。
