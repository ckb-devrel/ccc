...
| `locale`            | `ccc.ConnectorLocaleLike`                                              | 界面语言（BCP 47 标签，如 `"zh-CN"`），默认 `"en"` |
...
| `setClient`  | `(owner: ccc.Owner<ccc.Client>) => void` | 转移并切换 CKB Client                                  |
...
| `locale`     | `ccc.ConnectorLocaleLike`      | 连接器当前使用的界面语言                                |
...
**适用场景：** 你的应用是多语言的，希望连接器弹窗跟随应用语言。

连接器界面默认英文。传入 `locale` 即可切换语言；重新渲染 `Provider` 并传入新值，弹窗立即跟随。

```tsx
<ccc.Provider locale="zh-CN">{children}</ccc.Provider>
```

内置语言：`en`、`zh-Hans`。支持传入任意 BCP 47 标签，会自动解析到最接近的内置语言——优先同语言同文字，其次同语言（`zh-CN` 和 `zh-HK` 都会解析到 `zh-Hans`），最后回退到英文，因此直接传 `navigator.language` 也是安全的。钱包与链名（`MetaMask`、`JoyID`、`BTC` 等）是品牌名，不做翻译。

直接使用 Web Component 时用法相同：

```html
<ccc-connector locale="zh-CN"></ccc-connector>
```

翻译内置于连接器中，不支持应用层覆盖。这样无论连接器界面如何迭代，应用升级 CCC 后都能保证展示正确。如果发现翻译有误，请[提交 issue](https://github.com/ckb-devrel/ccc/issues/new)；如需新增语言，请参考[贡献指南](https://github.com/ckb-devrel/ccc/blob/master/CONTRIBUTING.md#contributing-a-connector-language)。

连接器不会自动检测浏览器语言。如果需要，请自行传入 `locale={navigator.language}`。

## 选择钱包网络

钱包集成会为每个兼容的可切换网络分别提供 Signer，例如 `BTC`、`BTC Testnet` 和 `BTC Signet`。用户直接在连接器中选择网络；必要时，`connect()` 会请求钱包切换网络。CKB 主网 Client 只展示与主网兼容的 Signer。
...
断开连接后选择目标网络对应的 Signer，例如 `BTC` 或 `BTC Signet`。
