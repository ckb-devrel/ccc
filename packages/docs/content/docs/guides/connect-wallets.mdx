...
| `locale`            | `ccc.ConnectorLocaleLike`                                              | UI language (BCP 47 tag, e.g. `"zh-CN"`), default `"en"` |
...
| `setClient`  | `(owner: ccc.Owner<ccc.Client>) => void` | Transfer and switch to a different CKB client |
...
| `locale`     | `ccc.ConnectorLocaleLike`      | Active UI language of the connector                    |
...
**Use this when:** your app is multilingual and the connector modal should follow it.

The connector UI is English by default. Pass a `locale` to switch languages; re-render the `Provider` with a new value and the modal updates immediately.

```tsx
<ccc.Provider locale="zh-CN">{children}</ccc.Provider>
```

Built-in locales: `en`, `zh-Hans`. Any BCP 47 tag is accepted and resolved to the closest built-in locale — same language and script first, then same language (`zh-CN` and `zh-HK` both → `zh-Hans`), then English — so passing `navigator.language` is safe. Wallet and chain names (`MetaMask`, `JoyID`, `BTC`, …) are brand names and are never translated.

Plain Web Component usage works the same way:

```html
<ccc-connector locale="zh-CN"></ccc-connector>
```

Translations ship inside the connector and are not overridable per app. This keeps every app rendering correctly whenever you upgrade CCC, even if the UI changes. If a translation is wrong, please [open an issue](https://github.com/ckb-devrel/ccc/issues/new); to add a language, see the [contributing guide](https://github.com/ckb-devrel/ccc/blob/master/CONTRIBUTING.md#contributing-a-connector-language).

Locale is not auto-detected from the browser. If you want that, pass `locale={navigator.language}` yourself.

## Choose a wallet network

Wallet integrations expose a separate signer for every compatible network they can switch to, such as `BTC`, `BTC Testnet`, and `BTC Signet`. The user selects the network directly in the connector; `connect()` then asks the wallet to switch when necessary. CKB mainnet clients only show mainnet-compatible signers.
