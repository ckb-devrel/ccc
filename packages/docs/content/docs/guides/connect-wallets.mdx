...
| `locale`            | `ccc.ConnectorLocale`                                                  | UI language (BCP 47 tag, e.g. `"zh-CN"`), default `"en"` |
...
| `locale`     | `ccc.ConnectorLocale`          | Active UI language of the connector                    |
...
Built-in locales: `en`, `zh-CN`. Unknown tags fall back to the closest registered language (`zh-HK` → `zh-CN`), then to English, so passing `navigator.language` is safe. Wallet and chain names (`MetaMask`, `JoyID`, `BTC`, …) are brand names and are never translated.
