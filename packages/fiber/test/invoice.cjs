  ...
  try {
    ...
    try {
      ...
      const response = await sdk.invoice.newInvoice({
        ...
        paymentPreimage: generatePaymentPreimage(),
        hashAlgorithm: "Sha256",
      });
      console.log("Invoice created successfully:");
      console.log("Invoice Address:", response.invoiceAddress);
      console.log("Payment Hash:", response.invoice?.data?.paymentHash);
      console.log("Amount:", response.invoice?.amount);
...
async function testGetInvoice() {
  try {
    ...
    try {
      ...
      const paymentHash =
        invoice.invoice?.data?.paymentHash ?? invoice.paymentHash;
      const invoiceInfo = await sdk.invoice.getInvoice(paymentHash);

      console.log("\nInvoice details:");
      console.log("Status:", invoiceInfo.status);
      console.log("Invoice Address:", invoiceInfo.invoiceAddress);
      console.log("Payment Hash:", invoiceInfo.invoice?.data?.paymentHash);
      console.log("Amount:", invoiceInfo.invoice?.amount);
      console.log("Description:", invoiceInfo.invoice?.data?.description);
      console.log("Expiry:", invoiceInfo.invoice?.data?.expiry);
    } catch (error) {
      if (error.error) {
        handleRPCError(error);
      } else {
...
async function testCancelInvoice() {
  try {
    ...
    try {
      ...
      const cancelledInvoice = await sdk.invoice.getInvoice(paymentHash);
      console.log(
