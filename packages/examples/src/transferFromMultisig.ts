...
for (const multisigSigner of multisigSigners) {
  ...
  if (signaturesCount == null) {
    console.log(
      `Need ${await multisigSigner.getMemberThreshold()} signatures, ${await multisigSigner.getMemberCount()} members in total`,
    );
  } else {
    console.log(
      `${signaturesCount}/${await multisigSigner.getMemberCount()} signers signed, need ${(await multisigSigner.getMemberThreshold()) - signaturesCount} more`,
    );
  }
}

if (await multisigSigners[0].needMoreSignatures(tx)) {
  console.log("No enough signatures");
} else {
