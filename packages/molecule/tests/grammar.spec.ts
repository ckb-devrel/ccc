...
import { parseMolecule } from "../src/barrel";

test("should parse sample", () => {
  const result = parseMolecule(`
    /* Basic Types */
  ...
  `,
    {
      extraReferences: {
        OutPoint: ccc.mol.Codec.from({
          ...
          decode: (buffer) => ccc.OutPoint.decode(buffer),
        }),
      },
    },
  );
  expect(
    result.OutPointVec.decode(
...
test("union with custom id", () => {
  const withCustomId = parseMolecule(`
    array Uint8 [byte; 1];
    array Uint16 [byte; 2];
    array Uint32 [byte; 4];
  ...
  expect(withCustomId.JSNumber.encode({ type: "Uint8", value: 1 })).toEqual(
    // prettier-ignore
    Uint8Array.from([
  ...
  const withoutCustomId = parseMolecule(`
    array Uint8 [byte; 1];
    array Uint16 [byte; 2];
    array Uint32 [byte; 4];
  ...
  expect(withoutCustomId.JSNumber.encode({ type: "Uint8", value: 1 })).toEqual(
    // prettier-ignore
    Uint8Array.from([
...
test("should parse blockchain.mol", () => {
  ...
  `,
    { extraReferences: {}, skipDependenciesCheck: false },
  );
  expect(result.Uint8.decode("0x01")).toEqual(1);
});
