...
[JoyID](https://joy.id/) 是一款基于 WebAuthn/Passkey 的钱包，无需助记词，通过设备生物识别（指纹、Face ID 等）管理密钥。

`@ckb-ccc/joy-id` 是 CCC 中负责集成 JoyID 钱包的协议支持层包。该包通过弹窗（popup）的方式与 JoyID 应用通信，为 CKB、Bitcoin、EVM 和 Nostr 四种链类型提供统一的 `Signer` 接口实现。
...
| 包名 | 说明 |
|------|------|
| `@ckb-ccc/core` | CCC 核心层，提供 `Signer`、`Client`、`Transaction` 等基础组件 |
...
<Callout type="info">
  如果你使用 `@ckb-ccc/connector-react` 或 `@ckb-ccc/ccc`，JoyID 已经包含在内——无需单独安装。
</Callout>
...
原生 CKB 链 Signer ，基于 JoyID Passkey 技术，支持主密钥（`main_key`）和子密钥（`sub_key`，依赖 COTA 协议）。
...
Bitcoin 链 Signer ，支持 P2WPKH（Native SegWit）和 P2TR（Taproot）两种地址类型。
...
EVM 链 Signer ，通过 JoyID 获取以太坊地址并签名。
...
Nostr 协议 Signer ，通过 JoyID 的弹窗流程来获取 Nostr 公钥并对 Nostr 事件进行签名。
...
这是推荐的集成入口，一次性返回所有 JoyID 支持的 Signer 列表。
...
`createPopup()` 是所有 Signer 与 JoyID App 通信的底层函数。 
...
如果你的项目使用 Lumos SDK，可以通过 Lumos 补丁来支持 JoyID：

```typescript
import { generateDefaultScriptInfos } from "@ckb-ccc/lumos-patches";

// 在使用 Lumos 之前调用一次，注册后不再需要 @ckb-lumos/joyid
...
1. **仅限浏览器环境**：所有 Signer 依赖 `window.open`、`window.localStorage`、`window.postMessage`，不支持 Node.js 服务端环境。
